...
import { PageHeader, Button, Container, Icon, Typography } from "../../index";
...
- **Layout:** Root is a `<header>`. Inside, a flex row (`.cp-page-header-row`) has start (title + subtitle) and actions (CTA + more trigger). The actions column uses `margin-left: auto` so it stays aligned to the right edge.
...
- **More menu:** When `moreMenuItems` is provided, the component renders a Dropdown with an icon Button (more_vert) and a list of buttons; each item’s `onClick` runs and the dropdown closes. When `moreMenuContent` is provided, that content is rendered inside the dropdown panel. Only one of `moreMenuItems` or `moreMenuContent` should be used.
...
- **Button** – Typically used for `primaryCta`
- **Typography** – Used internally for string title and subtitle
- **Dropdown** – Used internally for the more menu
- **Icon** – Used for the more_vert trigger icon
- **Container** – Use to wrap PageHeader and page content for layout and padding

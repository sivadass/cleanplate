...
The `Avatar` component displays user initials, an image, or a Material icon. It supports sizes, spacing (margin), and click events. Backgrounds are applied via CSS modules; use **`className`** to override—**`style`** is not part of the Avatar API.
...
- **Backgrounds:** Initials and icon use `var(--primary-brand)`; image mode uses `var(--white)` behind the photo for transparent assets
- **Sizes:** `small`, `medium`
- **Spacing:** Margin uses the suffix API (e.g. `"0"` → m-0, `"2"` → m-2)
- **Interactive:** Optional `onClick`; root is a `div` with standard attributes except `style`
...
When only `name` is provided, the avatar shows initials and `title`; fill comes from the default theme token (override with **`className`** + CSS).
...
- **Styling:** No inline **`style`** on the root; use **`className`** and your own CSS to customize appearance.

...
The `Dropdown` component is a flexible floating UI element that displays content relative to a trigger element. It uses custom positioning with flip and shift so the panel stays in the viewport, and supports multiple trigger modes (element, render prop, or label).

## Features

- **Positioning**: Places content relative to the trigger with configurable placement (12 options) and offset
- **Collision handling**: Optional flip and shift so the dropdown stays in viewport
- **Accessibility**: ARIA attributes (`aria-expanded`, `aria-haspopup`), Escape to close, click-outside to close
- **Flexible trigger**: Use a `trigger` element, `renderTrigger` function, or `triggerLabel` for a default button
- **Content**: Any React element as content; receives `onClose` and `className` when cloned
- **Customizable behavior**: Control close-on-outside, close-on-escape, flip, shift
...
const ActionMenu = () => {
  const ActionMenuContent = ({ onClose }) => {
    ...
    return (
      <div style={{ padding: "var(--space-2)" }}>
        <Typography variant="small" margin="m-0 m-b-2" color="var(--text-muted)">
          Actions
        </Typography>
        <MenuList 
          items={menuItems}

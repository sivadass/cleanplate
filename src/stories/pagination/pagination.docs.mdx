...
- **Design system**: Uses Container, Button (`size="small"`), Typography, Icon, and FormControls.Select (`size="small"`). Pagination has no `size` prop.
...
Page buttons and the rows-per-page Select are locked to Button / Select **`small`** (32px height, `--cp-form-control-radius-small` / 8px). Pagination does not expose `size`.
...
- **Button** – Used for prev/next and page numbers (`size="small"`).
- **FormControls.Select** – Used for the “Show per page” dropdown (`size="small"`).

...
The `AppShell` component provides a full-page layout that combines an optional **Header** (top), optional **Footer** (bottom), and an optional **MenuList** as a left **sidebar**, with the main content in the center. Use it for dashboard-style apps where the sidebar holds primary navigation and the header holds logo and utilities (e.g. user account **Dropdown** with **Avatar** trigger—same **`headerRight`** pattern as **`docs/Dropdown.md`** and standalone **Header**).

## Features

- **Header slot:** Optional. Pass Header props (object) or custom ReactNode. Renders full width at the top. When using **`header`**, **`headerRight`** should follow the same account-menu layout as a standalone **`Header`** (**`docs/Dropdown.md`** — *Recommended: account / user menu content*).
...
- **Responsive:** Sidebar column is hidden below 1024px. With a `HeaderProps` header, the default is to use the header’s mobile menu for the same items (`mobileSidebarDrawer` defaults off). Without that, AppShell uses a **Floating UI** mobile drawer (trigger, portal, overlay with scroll lock, modal focus). On desktop, set **`showCenterMenu: false`** on Header when the sidebar shows the same links so the header center is not duplicated (or use **`headerCenter`** for a title).
  ...
  return (
    <AppShell
      ...
      header={{
        ...
        // Recommended: Dropdown + Avatar + user meta + MenuList — see docs/Dropdown.md
...
- **Header / footer props:** When `header` or `footer` is a plain object (with `menuItems` for header or footer-specific keys), the component renders `<Header {...header} />` or `<Footer {...footer} />`. Otherwise it renders the ReactNode as-is. **`header.headerRight`** should match standalone **`Header`** patterns (**`docs/Dropdown.md`** for account menus).
...
- **Responsive / mobile nav:** At viewport width ≤ 1024px the sidebar column is hidden (CSS). When `header` is a Header props object, `mobileSidebarDrawer` defaults to `false` so you use the header’s mobile menu. Otherwise, with `sidebar` set, a Floating UI drawer (same MenuList) opens from a fixed trigger. Override with `mobileSidebarDrawer` / `mobileSidebarDrawerLabel`.
- **No spacing props on root:** AppShell does not expose margin/padding; use `className` or `contentClassName` for layout tweaks.

## Props

<ArgTypes of={AppShell} />

## Related components

- **Header** – Rendered in the header slot when `header` is a Header props object; often used with the same menu items as the sidebar for mobile. **`headerRight`** user menus: **`docs/Dropdown.md`**
...
- **Dropdown**, **Avatar**, **Typography** – Account **`headerRight`** pattern in **`docs/Dropdown.md`**; Avatar and Typography are also common in main content

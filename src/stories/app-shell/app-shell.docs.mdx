...
- **Responsive:** Sidebar column is hidden below 1024px. With a `HeaderProps` header, the default is to use the header’s mobile menu for the same items (`mobileSidebarDrawer` defaults off). Without that, AppShell uses a **Floating UI** mobile drawer (trigger, portal, overlay with scroll lock, modal focus).

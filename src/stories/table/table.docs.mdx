...
- **Column config**: Define columns with `id`, `title`, `textAlign`, `verticalAlign`, `widthPercentage`, and optional `customRender` for custom cell content
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Horizontal alignment for header and cells; default <code>"left"</code></td>
    </tr>
    <tr>
      <td><code>verticalAlign</code></td>
      <td><code>"top" | "middle" | "bottom"</code></td>
      <td>Vertical alignment for header and cells; defaults to table <code>cellVerticalAlign</code> (<code>"top"</code>)</td>
    </tr>
    <tr>
...
- **rowsPerPage**, **rowsPerPageOptions**, **onRowsPerPageChange**, **totalLabel**: Same as the [Pagination](/docs/atoms-pagination-documentation--docs) component. Built-in Pagination chrome is locked to Button / Select **`small`** (32px); Pagination has no `size` prop.

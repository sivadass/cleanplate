...
### Block Display
```jsx
<Container display="block" showBorder padding="3">
...
`padding`, `margin`, and `gap` accept the **spacing suffix** (e.g. `"0"`–`"9"`, `"auto"`, `"x-2"`, `"b-3"`). The component applies the `p-`, `m-`, or `g-` prefix internally.

### Padding
```jsx
// Single padding value (suffix: "2" → p-2, "4" → p-4)
<Container padding="2">Small padding</Container>
<Container padding="4">Medium padding</Container>
<Container padding="6">Large padding</Container>

// Multiple padding values (array of suffixes)
<Container padding={["2", "4"]}>Custom padding</Container>
```

### Margin
```jsx
// Single margin value (suffix: "2" → m-2, "4" → m-4)
<Container margin="2">Small margin</Container>
<Container margin="4">Medium margin</Container>

// Multiple margin values
<Container margin={["2", "4"]}>Custom margin</Container>
```
...
const DashboardLayout = () => {
  return (
    <Container display="flex" gap="4">
      ...
      {/* Main Content */}
      <Container width="three-quarters" showBorder padding="4">
        <Typography variant="h3" margin="b-4">
          Dashboard
        </Typography>
        <Container display="flex" gap="3" margin="b-4">
          <Container width="half" showBorder padding="3">
...
3. **Flex for Layouts**: Use flex display for complex layouts
   ```jsx
   // Good: Flex layout
   <Container display="flex" justify="space-between" gap="2">
